...
export default function SelectSchedule({ data, selectedSchedule, setSelectedSchedule, isShort, onCohortSelect, selectedCohortId }: Props) {
  ...
  return (
    <section className='flex flex-col gap-1'>
      {
        selectedSchedule ? (
          <p className='text-zuccini font-semibold flex flex-col md:flex-row gap-2 md:items-center'>
                      ...
                      <div className="flex flex-col md:flex-row md:items-center gap-2">
                        <div className="flex items-center gap-2 text-sm">
                          {cohortItem.schedule?.clases?.horas?.[0] && (
                            <span className="text-zuccini">{cohortItem.schedule.clases.horas[0]}</span>
                          )}
