  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2">
        <Tag className="w-5 h-5 text-zuccini" />
        ...
        <form onSubmit={handleSubmit} className="flex gap-2 items-stretch">
          <div className="flex-1 relative">
            <input
              ...
              className={`w-full py-3 px-4 border rounded-lg focus-visible:ring-2 focus-visible:ring-zuccini focus-visible:outline-none bg-zinc-800 text-white placeholder:text-gray-500 transition-all ${
                error
                  ? 'border-red-500 focus-visible:ring-red-500'
                  : success
                  ? 'border-emerald-500 focus-visible:ring-emerald-500'
                  : 'border-zinc-700 focus-visible:border-zuccini'
              }`}
              type="text"
              disabled={loading}
            />
            {loading && (
              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                <Loader2 className="w-5 h-5 animate-spin text-zuccini" />
              </div>
            )}
          </div>
          <button
            ...
            className="px-6 py-3 bg-zuccini hover:bg-blue-600 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-colors flex-shrink-0"
          >
