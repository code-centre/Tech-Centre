...
export default function ScheduleSection({ data, saveChanges }: Props) {
  ...
  useEffect(() => {
    if (data && data.schedule) {
      const scheduleData = Array.isArray(data.schedule) ? data.schedule : [data.schedule]
  ...
  const saveSchedule = () => {
    if (editingIndex !== null) {
      ...
      if (saveChanges) {
        saveChanges("schedule", updatedSchedules)
      }
  ...
  return (
    <section className="w-full py-16 bg-[var(--background)] text-white">
      <div className="container mx-auto px-4">
        ...
        <div className="flex justify-center gap-6 w-full max-w-6xl mx-auto">
        ...
        {schedules.length === 0 && (
          <div className="flex justify-center mt-8">
            <p className="text-gray-300">No hay horarios disponibles en el momento</p>
          </div>
        )}
      </div>
        {user?.rol === 'admin' && (
